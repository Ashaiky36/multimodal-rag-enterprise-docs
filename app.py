...
# Custom CSS - minimal, just for sidebar and styling
st.markdown("""
<style>
    .main { background-color: #f8fafc; }
    [data-testid="stSidebar"] { background-color: #0f172a; padding: 2rem 1rem; }
...
# Chat interface - Using Native Streamlit Chat Elements
if not st.session_state.processing_complete:
    st.info("👈 Upload a document and click 'Process Document' to begin.")
else:
            ...
            if message.get("sources"):
                with st.expander("📚 View Sources"):
                    for source in message["sources"]:
                        ...
                        **Source {source.get('index', '?')}** | {source.get('type', 'text').upper()} | Similarity: {source.get('similarity', 0):.3f}
                        """)
                        st.caption(source.get('content_preview', source.get('content', ''))[:300])
    
    # Input area - Using native Streamlit chat input
